// 排班視圖邏輯（對應舊版 schedule.js）
import { state } from './state.js';
import {
  parseTime, formatTime, formatDuration, getTaskHash,
...
export function renderSidebar() {
  ...
  state.staffData.forEach((staff, index) => {
    const staffRegion = staff.region || (state.REGIONS.length > 0 ? state.REGIONS[0] : '未分類');
    if (!state.currentRegion.includes('All') && !state.currentRegion.includes(staffRegion)) return;
    if (state.showWorkingOnly && staff.attendance === false) return;
...
export function renderTracksOnly() {
  ...
  const totalWidth = (closeH * 60 - startOfDay) * state.PX_PER_MIN;
  ...
  for (let h = openH; h <= closeH; h++) {
    const left = (h * 60 - startOfDay) * state.PX_PER_MIN;
...
function renderSingleTrack(container, content, staffId, startOfDay, endOfDay) {
  ...
  const taskStatuses = (staff && staff.taskStatuses) || {};
  ...
  lines.forEach((line, index) => {
    ...
    if (match) {
      ...
      if (startTime !== null) {
        ...
        tasks.push({
          start: startTime, end: endTime, rawText: trimmedLine, conflict: false,
          staffId, taskId, scheduledStart, scheduledEnd,
  ...
  tasks.forEach(task => {
    if (task.start > currentCursor) {
      const gap = task.start - currentCursor;
      if (gap >= 10) {
        const label = buildFreeBlockLabelHTML(
          formatDuration(gap), formatTime(currentCursor), formatTime(task.start),
        );
        createBlock(container, currentCursor, task.start, startOfDay, 'free', label, staffId);
      }
    }
    const type = task.conflict ? 'conflict' : 'work';
    createBlock(
      container, task.start, task.end, startOfDay, type,
      task.rawText, task.staffId, task.taskId,
      task.scheduledStart, task.scheduledEnd, task.inTime, task.outTime, task.lineIndex,
    );
    if (task.end > currentCursor) currentCursor = task.end;
  });

  if (currentCursor < endOfDay) {
    const gap = endOfDay - currentCursor;
    if (gap >= 10) {
      const label = buildFreeBlockLabelHTML(
        formatDuration(gap), formatTime(currentCursor), formatTime(endOfDay),
      );
...
function createBlock(container, start, end, offsetStart, type, content, staffId, taskId, scheduledStart, scheduledEnd, inTime, outTime, lineIndex) {
  if (end <= start) return;
  const left = (start - offsetStart) * state.PX_PER_MIN;
  const width = (end - start) * state.PX_PER_MIN;
  const div = document.createElement('div');
  ...
  const staff = state.staffData.find(s => s.id === staffId);
  const staffRegion = staff ? (staff.region || (state.REGIONS.length > 0 ? state.REGIONS[0] : '未分類')) : '未分類';
  const regionColor = getRegionColor(staffRegion);
    ...
    if (type === 'work') {
      div.onclick = function(e) {
        ...
        if (typeof window.openTimeModal === 'function') {
          window.openTimeModal(div, staffId, taskId, content, scheduledStart, scheduledEnd, lineIndex);
        }
      };
...
export function updateTimeLineAndClock() {
  ...
  const isToday = state.currentActiveDate === getTodayDateStr();
  if (isToday && offsetMins >= 0 && line) {
    line.style.display = 'block';
    line.style.left = (offsetMins * state.PX_PER_MIN) + 'px';
  } else if (line) {
...
function checkScheduleAlerts(currentMinutes) {
  if (state.currentActiveDate !== getTodayDateStr()) return;
  ...
  state.staffData.forEach(staff => {
    ...
    lines.forEach(line => {
      ...
      const taskUniqueId = `${state.currentActiveDate}-${staff.id}-${startMins}-${endMins}`;

      if (currentMinutes === preStartMins && !alertedTasks.has(taskUniqueId + '_PRE')) {
        playLoudAlarm();
        sendSystemNotification('⏳ 即將開始', `${displayName} 還有 5 分鐘開始 (${formatTime(startMins)})`);
        showToast(`⏳ ${displayName} 還有 5 分鐘！`);
        alertedTasks.add(taskUniqueId + '_PRE');
      }
      if (currentMinutes === startMins && !alertedTasks.has(taskUniqueId + '_START')) {
        playLoudAlarm();
        sendSystemNotification('🟢 服務開始', `${displayName} 開始 (${formatTime(startMins)})`);
        showToast(`🟢 ${displayName} 開始！`);
        alertedTasks.add(taskUniqueId + '_START');
      }
      if (currentMinutes === endMins && !alertedTasks.has(taskUniqueId + '_END')) {
        playLoudAlarm();
        sendSystemNotification('🔔 服務結束', `${displayName} 時間到 (${formatTime(endMins)})`);
        showToast(`🔔 ${displayName} 時間到！`);
        alertedTasks.add(taskUniqueId + '_END');
      }
    });
  });
}

export function renderScheduleAll() {
...
function startColResize(clientX) {
  ...
  isColDragging = true;
  colStartX = clientX;
  colStartWidth = parseInt(window.getComputedStyle(leftPanel).width, 10);
...
let rowStartY, rowStartHeight;
let currentStaffCardFn = null;
let currentTrackRowFn = null;

export function initRowResize(event, staffId) {
  ...
  if (currentStaffCardFn) {
    rowStartHeight = parseInt(window.getComputedStyle(currentStaffCardFn).height, 10);
  }
...
function moveRowResize(clientY) {
  ...
  const diff = clientY - rowStartY;
  let newHeight = rowStartHeight + diff;
...
function endRowResize() {
  if (isRowDragging) {
    ...
    if (currentRowId && currentStaffCardFn) {
      ...
      state.staffData = state.staffData.map(s => s.id === currentRowId ? { ...s, height: finalHeight } : s);
      if (typeof window.saveScheduleData === 'function') window.saveScheduleData();
    }
    currentRowId = null;
    currentStaffCardFn = null;
    currentTrackRowFn = null;
  }
}

export function initScheduleDragHandlers() {
...
export async function quickPaste(staffId) {
  try {
    const text = await navigator.clipboard.readText();
    if (!text || text.trim() === '') {
      showToast('⚠️ 剪貼簿裡面沒有文字喔');
      return;
    }

    const staff = state.staffData.find(s => s.id === staffId);
    if (staff && staff.content && staff.content.trim() !== '') {
    ...
    if (document.getElementById('view-settle').classList.contains('active')
        && typeof window.renderSettlementTable === 'function') {
